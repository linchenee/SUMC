...
% Reference: [2] J. Li, W. Cui, and X. Zhang, “Projected gradient descent for spectral compressed sensing 
% via symmetric Hankel factorization,” IEEE Trans. Signal Process., vol. 72, pp. 1590–1606, 2024.
% https://github.com/Jinshengg/SHGD/blob/main/SHGD.m
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for k = 1:nds
  ...
  % SVD of the initial estimated Hankel matrix
    ...
    % z = HankelVecMul(y_s, [n_s, n_s], r, nd, n_s, n_s)
    %% 
...
%         errors_SHGD(i_kappa, t) = error;   
       ...
       step(t) = stepsize;
        ...
        %% check the condition
        if ~isfinite(upd_error)|| ((upd_error < tol) && (t>=100)) ||upd_error>thresh_up
        % if ~isfinite(upd_error)|| ((upd_error < tol) ) ||upd_error>thresh_up
        % if upd_error < tol
            % fprintf('%d\n',t);
            break;
